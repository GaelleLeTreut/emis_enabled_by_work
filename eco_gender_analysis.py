...
with open(OUTPUTS_PATH+'decomposition_variance.tex','w') as file:
    file.write('% file automatically generated by ' + this_file + eof)
    file.write('% decomposition of variance of log of emission-based' + eof)
    file.write('\\intertext{or in numbers}'+eof)
    file.write("{:.2f}".format(np.sum(matrix)) + " &= " + "{:.2f}".format(matrix[0,0]) +" &&+ " + "{:.2f}".format(matrix[1,1]) + " &&+ 2*" +"{:.2f}".format(matrix[0,1]) + eof)
    file.write('\\intertext{which can also be expressed in relative terms}'+eof)
...
with open(OUTPUTS_PATH+'section_inequalities_numbers.tex','w') as file:
    file.write('% file automatically generated by ' + this_file + eof)
    file.write('% numbers used in main text' + eof)
    file.write('\\newcommand\\relativeratioofvariance{' + "{:.2f}".format(matrix[0,0]/matrix[1,1]) + '} %ratio of variance of log of emissions divided by variance of log of wages' + eof)
...
with open(OUTPUTS_PATH+'section_inequalities_numbers.tex','a') as file:
    file.write('\\newcommand\\ratioofemissionshighestlowest{' + "{:.0f}".format(bts.full_insee_table.loc[highest_emitting_person, 'income-based_emissions']/bts.full_insee_table.loc[lowest_emitting_person, 'income-based_emissions']) + '}' +eof)
    file.write('\\newcommand\\ratioofwageshighestlowest{' + "{:.0f}".format(bts.full_insee_table.loc[highest_emitting_person, 'salary_value']/bts.full_insee_table.loc[lowest_emitting_person, 'salary_value']) + '}' +eof)
...
if ((23 in bts.full_insee_table[bts.full_insee_table['A35']==lowest_emitting_sector]['TRNNETO'].unique()) and (0 in bts.full_insee_table[bts.full_insee_table['A35']==highest_emitting_sector]['TRNNETO'].unique() )):
    with open(OUTPUTS_PATH+'section_inequalities_numbers.tex','a') as file:
        file.write('\\newcommand\\ratioofemissions{' + "{:.2f}".format(((bts.dic_TRNNETO_to_salary[23]*bts.dic_to_emission_content_A35[lowest_emitting_sector])/(bts.dic_TRNNETO_to_salary[0]*bts.dic_to_emission_content_A35[highest_emitting_sector]))[0]) + '}' +eof)
        file.write('\\newcommand\\ratioofwages{'+"{:.2f}".format((bts.dic_TRNNETO_to_salary[23]/bts.dic_TRNNETO_to_salary[0])[0])+'}' +eof)
else:
