...
carbon_intensity_by_class = ut.stat_data_generic(['TRNNETO'],bts.full_insee_table, ut.mean_emission_content)
for i in range(24):
    carbon_intensity_by_class.loc[i,'TRNNETO'] += 1
carbon_intensity_by_class.set_index(['TRNNETO'], inplace=True)

#lowest and highest value per income class
mean_carbon_intensity_by_class = carbon_intensity_by_class['mean emission content']['All']
min_carbon_intensity_by_class = min(carbon_intensity_by_class['mean emission content'])
min_gap_to_mean = 100*( 1- min_carbon_intensity_by_class/mean_carbon_intensity_by_class)
max_carbon_intensity_by_class = max(carbon_intensity_by_class['mean emission content'])
...
ut.df_to_csv_with_comment( carbon_intensity_by_class, OUTPUTS_PATH + 'carbon_intensity_by_wage_class.csv', '% file automatically generated by ' + this_file + eol + '% downstream carbon intensity by wage classes')

##plot now done with pgfplots in Latex
##plot for mean emission content by wage classes
#sns.set_context('paper', font_scale=0.9)
#plt.figure(figsize=(18, 12))
#sns.barplot(x=carbon_intensity_by_class['TRNNETO'], y="mean emission content", data=carbon_intensity_by_class,palette='deep')
...
#plt.savefig(OUTPUTS_PATH+'fig_relative_wage_share_and_intensity_by_industry.jpeg', bbox_inches='tight')
##plt.show()
#plt.close()

table_diff_pop.set_index(['A35'], inplace=True)
ut.df_to_csv_with_comment( table_diff_pop, OUTPUTS_PATH + 'diff_wage_share_by_branch.csv', '% file automatically generated by ' + this_file + eol + '% difference of wage share between male and female workers per sector')

with open(OUTPUTS_PATH+'section_inequalities_numbers.tex','a') as file:
    file.write('\\newcommand\\carbonintensityHZ{' + "{:.0f}".format(table_diff_pop.loc['HZ','mean emission content']) + '}' +eol)
